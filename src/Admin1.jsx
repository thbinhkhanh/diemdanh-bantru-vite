...
export default function Admin({ onCancel }) {
  ...
  const handleInitNewYearData = async () => {
    ...
    const classListDocs = ["K1", "K2", "K3", "K4", "K5", "TRUONG"];
    const otherCollections = ["BANTRU", "DIEMDANH", "NHATKYBANTRU"];

    try {
      // Tạo CLASSLIST_${selectedYear} với mỗi khối có document riêng { list: [] }
      for (const docName of classListDocs) {
        await setDoc(doc(db, `CLASSLIST_${selectedYear}`, docName), { list: [] });
      }

      // Tạo DANHSACH_${selectedYear} với 1 document ID rỗng { list: "" }
      await setDoc(doc(db, `DANHSACH_${selectedYear}`, ""), { list: "" });

      // Tạo các collection khác với 1 document ID rỗng { }
      for (const col of otherCollections) {
        await setDoc(doc(db, `${col}_${selectedYear}`, ""), {});
      }
