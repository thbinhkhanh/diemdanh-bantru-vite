...
export default function Lop1() {
  ...
  const [isClassSelectorDisabled, setIsClassSelectorDisabled] = useState(false);
  ...
  const fetchClassList = async (namHoc) => {
    try {
      ...
      // Nếu selectedClass đã có (ví dụ do đăng nhập), không ghi đè
      const account = localStorage.getItem("account");
      const preselectedClass = account && /^\d\.\d$/.test(account) ? account : null;

      if (preselectedClass && classes.includes(preselectedClass)) {
        setSelectedClass(preselectedClass);
        await fetchStudents(preselectedClass, namHoc);
        setIsClassSelectorDisabled(true); // ✅ khóa chọn lớp
